    ...
    def processMessage(self, msg):
        try:
            ...
            if "echo" in msg:
                ...
                return
            if 'self_id' in msg and msg['self_id'] != variable.bot_id:
                return
            else:
                ...
                if "status" in msg:
                    ...
                    if msg['post_type'] == 'meta_event':
                        ...
                        if msg['message_type'] == 'private':
                            ...
                            self.private_message(msg['time'], msg['self_id'], msg['sub_type'], msg['message_id'],
                                                 msg['user_id'], msg['message'], msg['raw_message'], msg['font'],
                                                 msg['sender'])
                        elif msg['message_type'] == 'group':
                            logger.info(
                                f"收到 [{msg['group_id']}]({msg['user_id']}) 在群 [{msg['group_id']}] 中的群消息: {msg['raw_message']}")
                            if msg['sub_type'] != 'anonymous':
                                msg['anonymous'] = None
                            self.group_message(msg['time'], msg['self_id'], msg['sub_type'], msg['message_id'],
                                               msg['group_id'], msg['user_id'], msg['anonymous'], msg['message'],
                                               msg['raw_message'], msg['font'], msg['sender'])
                        else:
                            logger.debug(msg)
                        ...
                        if msg['notice_type'] == 'group_upload':
                            ...
                            self.group_admin(msg['time'], msg['self_id'], msg['sub_type'], msg['group_id'],
                                             msg['user_id'])
                            ...
                            self.group_decrease(msg['time'], msg['self_id'], msg['sub_type'], msg['group_id'],
                                                msg['operator_id'], msg['user_id'])
                            ...
                            self.group_increase(msg['time'], msg['self_id'], msg['sub_type'], msg['group_id'],
                                                msg['operator_id'], msg['user_id'])
                        elif msg['notice_type'] == 'group_ban':
                            if msg['sub_type'] == 'ban':
                                logger.info(
                                    f"({msg['user_id']}) 在群 [{msg['group_id']}] 中被 ({msg['operator_id']}) 禁言 {msg['duration']} 秒")
                            else:
                                logger.info(f"({msg['user_id']}) 在群 [{msg['group_id']}] 中被 ({msg['operator_id']}) 解除禁言")
                            self.group_ban(msg['time'], msg['self_id'], msg['sub_type'], msg['group_id'],
                                           msg['operator_id'], msg['user_id'], msg['duration'])
                            ...
                            self.group_recall(msg['time'], msg['self_id'], msg['group_id'], msg['user_id'],
                                              msg['operator_id'], msg['message_id'])
                            ...
                            if msg['sub_type'] == 'poke':
                                self.group_poke(msg['time'], msg['self_id'], msg['group_id'], msg['user_id'],
                                                msg['target_id'])
                            elif msg['sub_type'] == 'lucky_king':
                                self.lucky_king(msg['time'], msg['self_id'], msg['group_id'], msg['user_id'],
                                                msg['target_id'])
                            elif msg['sub_type'] == 'honor':
                                self.honor(msg['time'], msg['self_id'], msg['group_id'], msg['honor_type'],
                                           msg['user_id'])
                            else:
                                logger.debug(msg)
                        else:
                        ...
                        if msg['request_type'] == 'friend':
                            ...
                            self.friend_request(msg['time'], msg['self_id'], msg['user_id'], msg['comment'],
                                                msg['flag'])
                        elif msg['request_type'] == 'group':
                            logger.info(f"({msg['user_id']}) 请求加入群 [{msg['group_id']}]")
                            self.group_request(msg['time'], msg['self_id'], msg['sub_type'], msg['group_id'],
                                               msg['user_id'], msg['comment'], msg['flag'])
                        else:
                            logger.debug(msg)
                    else:
    ...
    def get_sorted_func(self, method):
        ...
        for plugin in variable.loader.plugin_enables:
            if method in variable.loader.plugin_methods[plugin]:
                if hasattr(variable.loader.plugin_registers[plugin],
                           variable.loader.plugin_methods[plugin][method]['func']):
                    try:
                        funcs[variable.loader.plugin_methods[plugin][method]['priority']].append((plugin, getattr(
                            variable.loader.plugin_registers[plugin],
                            variable.loader.plugin_methods[plugin][method]['func'])))
                    except:
                        funcs[variable.loader.plugin_methods[plugin][method]['priority']] = [(plugin, getattr(
                            variable.loader.plugin_registers[plugin],
                            variable.loader.plugin_methods[plugin][method]['func']))]
        return funcs
    ...
    def private_message(self, time, self_id, sub_type, message_id, user_id, message, raw_message, font, sender):
        ...
        for priority in sorted(funcs.keys()):
            for plugin in funcs[priority]:
                ...
                if plugin[1](time, self_id, sub_type, message_id, user_id, message, raw_message, font,
                             sender) and priority > 10000:
    ...
    def group_message(self, time, self_id, sub_type, message_id, group_id, user_id, anonymous, message, raw_message,
        ...
        for priority in sorted(funcs.keys()):
            for plugin in funcs[priority]:
                ...
                if plugin[1](time, self_id, sub_type, message_id, group_id, user_id, anonymous, message, raw_message,
                             font, sender) and priority > 10000:
                    logger.debug(f"插件 {plugin[0]} 阻塞了消息")
                    return
