...
class Util(object):
    ...
    def get_group_member_info(self,auth,group_id,user_id,no_cache=False,timeout=5):
        if self.check_auth(auth,'get_group_member_info'):
            uid=uuid.uuid4().hex
            m={"action":"get_group_member_info","params":{"group_id":group_id,"user_id":user_id,"no_cache":no_cache},"echo":uid}
            m=json.dumps(m)
            variable.ws.send(m)
            ret=self.waitFor(uid,timeout=timeout)
            if ret['status']=='ok':
    ...
    def seg_image(self,file,type,cache=1,proxy=1,timeout=None):
        if file=="":
            ...
            if file.startswith('http'):
                if type=="flash":
                    if timeout is None:
                        return [{'type':'image','data':{'file':file,'type':'flash','cache':cache,'proxy':proxy}}]
                    else:
                        return [{'type':'image','data':{'file':file,'type':'flash','cache':cache,'proxy':proxy,'timeout':timeout}}]
                else:
                    if timeout is None:
                        return [{'type':'image','data':{'file':file,'cache':cache,'proxy':proxy}}]
                    else:
                        return [{'type':'image','data':{'file':file,'cache':cache,'proxy':proxy,'timeout':timeout}}]
            else:
                if type=="flash":
        ...
        if file=="":
            ...
            if file.startswith('http'):
                if timeout is None:
                    ...
                    return [{'type':'record','data':{'file':file,'magic':magic,'cache':cache,'proxy':proxy,'timeout':timeout}}]
            else:
        ...
        if file=="":
            ...
            if file.startswith('http'):
                if timeout is None:
                    ...
                    return [{'type':'video','data':{'file':file,'cache':cache,'proxy':proxy,'timeout':timeout}}]
            else:
    ...
    def seg_music(self,type,id=None,url=None,audio=None,title=None,content=None,image=None):
        if type=="custom":
            return [{'type':'music','data':{'type':type,'url':url,'audio':audio,'title':title,'content':content,'image':image}}]
        else:
